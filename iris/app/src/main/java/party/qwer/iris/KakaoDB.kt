...
class KakaoDB {
    ...
    // #4: highest chat_log id right now, captured as a baseline before sending so we
    // only consider rows created after the send when verifying delivery.
    fun latestLogId(): Long {
        return connection.rawQuery("SELECT MAX(_id) FROM chat_logs", null).use { cursor ->
            if (cursor.moveToFirst()) cursor.getLong(0) else 0L
        }
    }

    // #4: did the bot's own message actually land in this chat after `afterLogId`?
    ...
    fun verifyBotMessage(chatId: Long, text: String, afterLogId: Long): Boolean {
        return connection.rawQuery(
            ...
            while (cursor.moveToNext()) {
                val row: MutableMap<String, String?> = HashMap()
                for (columnName in columnNames) {
                    row[columnName] = cursor.getString(cursor.getColumnIndexOrThrow(columnName))
                }
                if (decryptRow(row)["message"] == text) {
                    return@use true
                }
            }
            false
